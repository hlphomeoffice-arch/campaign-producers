(() => {
  ...
  if (form) {
    ...
    form.addEventListener("submit", (event) => {
      ...
      const configuredFields = Array.from(form.querySelectorAll("[data-email-label]"))
        .filter((field) => field.name)
        .map((field) => [field.dataset.emailLabel, field.name]);
      ...
      const fields = configuredFields.length ? configuredFields : [
        ...
        ["What needs to happen?", "commercial_goal"],
